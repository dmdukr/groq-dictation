...
Uses a STANDALONE Tk instance in its own thread — intentionally separate from
tk_host to avoid segfaults caused by Tk + pystray + keyboard hooks running
simultaneously. This overlay is short-lived (recording duration only) and
doesn't use sv_ttk, so a separate Tk instance is safe.
"""
